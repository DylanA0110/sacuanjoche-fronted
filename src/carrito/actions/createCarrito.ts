...
export const createCarrito = async (data: CreateCarritoDto): Promise<Carrito> => {
  ...
  if (!data.idUser || typeof data.idUser !== 'string') {
    console.error('❌ [createCarrito] idUser inválido (debe ser UUID string):', data);
  ...
  try {
    console.log('🛒 [createCarrito] Creando carrito con payload:', JSON.stringify(payload, null, 2));
    
    const response = await carritoApi.post<Carrito>('/', payload);
    console.log('✅ [createCarrito] Carrito creado:', response.data.idCarrito);
    ...
    console.error('❌ [createCarrito] Error al crear carrito:', {
