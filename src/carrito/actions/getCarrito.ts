...
export const getCarritoActivo = async (): Promise<Carrito | null> => {
  try {
    ...
    console.log('📦 [getCarritoActivo] Respuesta del backend:', response.data);
    console.log('📦 [getCarritoActivo] Tipo de respuesta:', Array.isArray(response.data) ? 'Array' : typeof response.data);
    
    // Si la respuesta es un array
    if (response.data && Array.isArray(response.data)) {
      const firstItem = response.data[0];
      console.log('📦 [getCarritoActivo] Primer item:', firstItem);
      console.log('📦 [getCarritoActivo] Tiene idCarritoArreglo?', 'idCarritoArreglo' in firstItem);
      console.log('📦 [getCarritoActivo] Tiene carrito?', 'carrito' in firstItem);
      console.log('📦 [getCarritoActivo] Tiene carritosArreglo?', 'carritosArreglo' in firstItem);
      console.log('📦 [getCarritoActivo] carritosArreglo:', firstItem?.carritosArreglo);
      
      // Verificar si es un CarritoArreglo (tiene idCarritoArreglo y carrito anidado)
      if (firstItem && 'idCarritoArreglo' in firstItem && 'carrito' in firstItem) {
        console.log('✅ [getCarritoActivo] Respuesta es array de CarritoArreglo, construyendo Carrito...');
        console.log('📦 [getCarritoActivo] Primer item completo:', JSON.stringify(firstItem, null, 2));
        console.log('📦 [getCarritoActivo] Primer item.arreglo:', firstItem.arreglo);
        ...
        const carritosArregloMapeados = (response.data as any[]).map((item: any) => {
          console.log('🔄 [getCarritoActivo] Mapeando item:', {
        ...
        console.log('📦 [getCarritoActivo] CarritosArreglo mapeados:', carritosArregloMapeados);
        console.log('📦 [getCarritoActivo] Primer carritoArreglo mapeado:', carritosArregloMapeados[0]);
        console.log('📦 [getCarritoActivo] Primer carritoArreglo.arreglo:', carritosArregloMapeados[0]?.arreglo);
        ...
        console.log('✅ [getCarritoActivo] Carrito construido:', carrito);
        console.log('📦 [getCarritoActivo] Primer carritoArreglo final:', carrito.carritosArreglo?.[0]);
        console.log('📦 [getCarritoActivo] Primer carritoArreglo.arreglo final:', carrito.carritosArreglo?.[0]?.arreglo);
        return carrito;
      }
      
      // Si es un array de Carrito normal
      console.log('✅ [getCarritoActivo] Respuesta es array de Carrito');
      console.log('📦 [getCarritoActivo] Carrito completo:', JSON.stringify(firstItem, null, 2));
      ...
      if (carrito?.carritosArreglo && Array.isArray(carrito.carritosArreglo)) {
        console.log('📦 [getCarritoActivo] Verificando carritosArreglo...');
        console.log('📦 [getCarritoActivo] Total carritosArreglo:', carrito.carritosArreglo.length);
        
        // Verificar si algún carritoArreglo no tiene el objeto arreglo
        const necesitaCargarArreglos = carrito.carritosArreglo.some((item: any) => !item.arreglo);
        
        if (necesitaCargarArreglos) {
          console.log('📥 [getCarritoActivo] Algunos arreglos no tienen datos completos. Cargando...');
          ...
          const carritosArregloCompletos = await Promise.all(
            carrito.carritosArreglo.map(async (item: any) => {
              ...
              if (item.arreglo && item.arreglo.nombre && item.arreglo.url) {
                console.log(`✅ [getCarritoActivo] CarritoArreglo ${item.idCarritoArreglo} ya tiene datos completos`);
                return item;
              }
              
              // Si no tiene arreglo completo, cargar el carrito arreglo completo desde el endpoint
              if (item.idCarritoArreglo) {
                console.log(`📥 [getCarritoActivo] Cargando carrito arreglo ${item.idCarritoArreglo}...`);
                try {
                  const carritoArregloCompleto = await getCarritoArreglo(item.idCarritoArreglo);
                  console.log(`✅ [getCarritoArreglo] CarritoArreglo ${item.idCarritoArreglo} cargado:`, {
                  ...
                  console.error(`❌ [getCarritoActivo] Error al cargar carrito arreglo ${item.idCarritoArreglo}:`, error);
                  
                  // Fallback: intentar cargar solo el arreglo por ID
                  if (item.idArreglo) {
                    try {
                      console.log(`📥 [getCarritoActivo] Fallback: Cargando arreglo ${item.idArreglo} por ID...`);
                      const arregloCompleto = await getArregloById(item.idArreglo);
                      return {
                        ...item,
                        arreglo: {
                      ...
                      console.error(`❌ [getCarritoActivo] Error en fallback al cargar arreglo ${item.idArreglo}:`, fallbackError);
                    }
                  }
          ...
          console.log('✅ [getCarritoActivo] Carrito actualizado con arreglos completos');
          console.log('📦 [getCarritoActivo] Primer carritoArreglo actualizado:', carritosArregloCompletos[0]);
          console.log('📦 [getCarritoActivo] Primer carritoArreglo.arreglo actualizado:', carritosArregloCompletos[0]?.arreglo);
        } else {
          console.log('✅ [getCarritoActivo] Todos los arreglos ya tienen datos completos');
        }
      }
    ...
    if (response.data && 'data' in response.data) {
      ...
      if (data.data && data.data.length > 0) {
        ...
        if (firstItem && 'idCarritoArreglo' in firstItem && 'carrito' in firstItem) {
          console.log('✅ [getCarritoActivo] Respuesta paginada es array de CarritoArreglo, construyendo Carrito...');
          const carritoArreglo = firstItem as any;
          const carrito: Carrito = {
            idCarrito: carritoArreglo.carrito.idCarrito,
            idUser: carritoArreglo.carrito.idUser,
          ...
          console.log('✅ [getCarritoActivo] Carrito construido desde respuesta paginada:', carrito);
          return carrito;
        }
    ...
    console.log('⚠️ [getCarritoActivo] No se pudo procesar la respuesta');
    return null;
  } catch (error: any) {
