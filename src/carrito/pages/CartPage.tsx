...
function ArregloImage({
  ...
  >({
    ...
    queryFn: async () => {
      logger.debug('🖼️ [ArregloImage] Cargando media para arreglo:', idArreglo);
      try {
        const media = await getArregloMedia(idArreglo);
        logger.debug('✅ [ArregloImage] Media cargada:', media);
        return media;
      } catch (error) {
        logger.error('❌ [ArregloImage] Error al cargar media:', error);
        return [];
      }
    },
  ...
  if (hasDirectUrl) {
    ...
    const imagenPrincipal =
      mediaArray.find(
        (m: any) => m?.isPrimary && m?.url && m.url.trim() !== ''
  ...
  // Debug en desarrollo
  logger.debug('🖼️ [ArregloImage] Estado:', {
    idArreglo,
    hasDirectUrl,
    hasValidMedia,
    shouldLoadMedia,
      ...
      {isLoadingMedia ? (
        ...
        <div className="w-28 h-28 sm:w-32 sm:h-32 bg-gray-100 rounded-lg border border-gray-200 overflow-hidden shadow-sm">
          <img
            ...
            onError={(e) => {
              logger.error(
                '❌ [ArregloImage] Error al cargar imagen:',
                imagenUrl
              );
              ...
              if (parent && !parent.querySelector('.placeholder')) {
                ...
                placeholder.className =
                  'w-full h-full flex items-center justify-center bg-gray-100 placeholder';
                placeholder.innerHTML =
                  '<span class="text-xs text-gray-400 text-center px-2">Sin imagen</span>';
                parent.appendChild(placeholder);
              }
            }}
            onLoad={() => {
              logger.debug(
                '✅ [ArregloImage] Imagen cargada exitosamente:',
                imagenUrl
              );
            }}
          />
...
export default function CartPage() {
  ...
  const productos =
    carrito && Array.isArray((carrito as any).carritosArreglo)
  ...
  if (productos.length > 0) {
    logger.debug('🛒 [CartPage] Productos en carrito:', productos);
    productos.forEach((item: any, index: number) => {
      logger.debug(`📦 [CartPage] Producto ${index + 1}:`, {
                ...
                <div className="divide-y divide-gray-200">
                  {productos.map(
                    (item: {
                      ...
                      cantidad: number;
                      totalLinea: string;
                      idCarritoArreglo: Key | null | undefined;
                    }) => {
