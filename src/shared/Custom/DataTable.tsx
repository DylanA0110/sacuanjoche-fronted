...
export function DataTable({
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      {/* Table */}
      <div className="overflow-x-auto rounded-lg border-0 bg-white -mx-1 sm:mx-0 shadow-sm">
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50/90 hover:bg-gray-50 border-b-2 border-gray-200/80">
              {columns.map((col) => (
                <TableHead
                  key={col.key}
                  className="text-xs sm:text-sm font-bold text-gray-800 whitespace-nowrap px-4 sm:px-5 md:px-6 py-4 sm:py-5 uppercase tracking-wider"
                >
                  {col.label}
                </TableHead>
              ))}
              {(onView || onEdit || onDelete || customActions) && (
                <TableHead className="text-right text-xs sm:text-sm font-bold text-gray-800 whitespace-nowrap px-4 sm:px-5 md:px-6 py-4 sm:py-5 uppercase tracking-wider">
                  Acciones
                </TableHead>
              )}
            </TableRow>
          ...
          <TableBody>
            {isLoading ? (
              ...
              filteredData.map((item, idx) => (
                <TableRow
                  key={idx}
                  className="border-b border-gray-100/50 hover:bg-gray-50/50 transition-all duration-150"
                >
                  {columns.map((col) => (
                    <TableCell
                      key={col.key}
                      className="text-sm sm:text-base text-gray-800 bg-white whitespace-nowrap px-4 sm:px-5 md:px-6 py-4 sm:py-5 font-medium"
                    >
                      {col.render
                  ...
                  {(onView || onEdit || onDelete || customActions) && (
                    <TableCell className="text-right bg-white whitespace-nowrap px-4 sm:px-5 md:px-6 py-4 sm:py-5">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {onView && (
                          <Button
                            ...
                            variant="ghost"
                            onClick={() => onView(item)}
                            className="h-9 w-9 p-0 text-gray-700 hover:text-gray-900 bg-transparent hover:bg-gray-100/80 rounded-full transition-all duration-200 hover:scale-110 border-0"
                            title="Ver detalles"
                          >
                            <MdVisibility className="h-4 w-4 text-gray-700" />
                          </Button>
                        )}
                        {onEdit && (
                          <Button
                        ...
                        {onDelete && (
                          <Button
                            ...
                            className="h-9 w-9 p-0 text-red-600 hover:text-red-700 rounded-full hover:bg-red-50/80 transition-all duration-200 hover:scale-110"
                            title="Eliminar"
                          >
